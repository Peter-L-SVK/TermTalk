/*
 * TermTalk - Terminal Chat Server
 * Version: 0.8
 * Copyright (c) 2025-2026 Peter Leukanič
 * Under MIT License
 *
 * Multi-client chat server with broadcast messaging
 */
...
async fn main() -> std::io::Result<()> {
    ...
    let token_username_map: Arc<Mutex<HashMap<usize, String>>> =
    ...
    while let Ok((stream, _)) = listener.accept().await {
        ...
        log_message(
            &log_file,
            &format!("DEBUG: New client {} connected", client_token),
        )
        .await;
        ...
        tokio::spawn(async move {
                ...
                .is_err()
            {
                log_message(
                    &log_file_clone,
                    &format!("DEBUG: Failed to send token to client {}", client_token),
                )
                .await;
                return;
            }
            ...
            loop {
                username.clear();
                if write_to_stream(&write_stream, "Enter your username: ")
                    .await
                    .is_err()
                {
                    log_message(
                        &log_file_clone,
                        &format!(
                            "DEBUG: Failed to prompt client {} for username",
                            client_token
                        ),
                    )
                    .await;
                ...
                if reader.read_line(&mut username).await.is_err() {
                    log_message(
                        &log_file_clone,
                        &format!(
                            "DEBUG: Failed to read username from client {}",
                            client_token
                        ),
                    )
                    .await;
                ...
                // Check if the username is already in use by another client
                {
                        ...
                        .any(|existing_username| existing_username == &username)
                    {
                        ...
                        log_message(
                            &log_file_clone,
                            &format!("DEBUG: Username '{}' is already taken", username),
                        )
                        .await;
                        if write_to_stream(&write_stream, &error_message)
                            .await
                            .is_err()
                        {
                            log_message(
                                &log_file_clone,
                                &format!(
                                    "DEBUG: Failed to send error message to client {}",
                                    client_token
                                ),
                            )
                            .await;
                ...
                // Add the token-username mapping to the HashMap
                {
                    ...
                    log_message(
                        &log_file_clone,
                        &format!(
                            "DEBUG: Added token-username mapping: {} -> {}",
                            client_token, username
                        ),
                    )
                    .await;
                }
                ...
                if write_to_stream(&write_stream, &success_message)
                    .await
                    .is_err()
                {
                    log_message(
                        &log_file_clone,
                        &format!(
                            "DEBUG: Failed to send success message to client {}",
                            client_token
                        ),
                    )
                    .await;
                ...
                // Broadcast the welcome message - FIXED: Remove duplicate SERVER: prefix
                let welcome_message = format!("{} has joined the chat!", username);
                let _ = sender.send(("SERVER".to_string(), welcome_message));
            ...
            // Remove the token-username mapping when the client disconnects
            {
                ...
                log_message(
                    ...
                    &format!(
                        ...
                        client_token, username_clone_for_log
                    ),
                )
                .await;
            }
            ...
            log_message(
                &log_file_clone,
                &format!("DEBUG: Client {} disconnected", client_token),
            )
            .await;
        });
    }
